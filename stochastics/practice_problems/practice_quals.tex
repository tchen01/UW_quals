...
\begin{solution}[Solution]
\begin{enumerate}[label=(\alph*)]
    \item If we transition down, the probability of reaching state \( N \) before state 0 is \( a_N(i-1) \). Similarly, if we transition up, the probability of reaching state \( N \) before state 0 is \( a_N(i+1) \).
...
\begin{enumerate}[label=(\alph*)]
    \item Recall that \( X_t \) can be written explicitly in terms of \( W_t \) as,
        ...
        Write speed density,
        \begin{align*}
            m(y) = \exp \left( \int y \d y \right) = \exp(y^2/2)
        \end{align*}
        ...
        We now funch such eigenfunctions. Indeed, define,
        \begin{align*}
        ...
        \begin{align*}
            H_n' &= xH_n - H_{n+1} \\
%            H_n'' = xH_n' - H_{n+1}' = x^2H_n - xH_{n+1} - xH_{n+1} + H_{n+2}
            H_n'' &= nH_{n-1}' = nxH_{n-1}-nH_n
        \end{align*}
